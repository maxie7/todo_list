...
RSpec.describe CommentsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    context 'cancan doesnt allow :create' do
      before do
        ...
        post :create, text: comment.text
      end
      it { expect(response).to redirect_to(root_path) }
    end

    context 'success save' do
      before { post :create, text: comment.text }
  ...
  describe 'POST #update' do
    ...
    context 'success update' do
      before { patch :update, id: comment.id, text: 'some text' }

      context 'cancan doesnt allow :update' do
        before do
          ability.cannot :update, Comment
          patch :update, id: comment.id, text: 'new text'
        end
        it { expect(response).to redirect_to(root_path) }
      end
      ...
      it '#update comment' do
        expect(assigns(:comment)).to receive(:update).with(text: comment.text).and_return(true)
        patch :update, id: comment.id, text: comment.text
      end
    end

    context 'invalid update' do
      before do
        patch :update, id: comment.id, text: nil
      end
